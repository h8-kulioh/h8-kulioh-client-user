...
export default function UserTask() {
    ...
    const getBab = async () => {
        try {
            ...
            const matematika = response.data.filter(el => el.subject === `Matematika`)
            setBab(matematika)
        }
        catch (err) {
            console.log(err);
    ...
    const openSwal = (e) => {
        ...
        return (Swal.fire({
            ...
            <text x="50%" y="50%" dominant-baseline="central" text-anchor="middle">85%</text>
                ...
                + `lengkapi pembelajaran untuk bekal SBMPTN <br/>`
                + `<a href="">Lihat statistik lengkap</a>`
        })
        )
    }
    ...
    return (
        <>
            <Container className='mt-5'>
                ...
                <Button onClick={e => openSwal(e)} variant="outline-primary">Statistik</Button>
                <div className='mt-5'>

                    {
                        bab.map((x, num) => {

                            return <CardComponent key={x.id} number={num + 1} babName={x.name} listSub={tasks} />
                        })
                    }
