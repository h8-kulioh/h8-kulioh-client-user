...
const ProfilePage = () => {
    ...
    const getMajor = async (id1, id2) => {
        try {
            console.log(id1, id2);
            const response1 = await axios.get(`http://localhost:3001/majorsroute/major/?UniversityId=${id1}`)
            const response2 = await axios.get(`http://localhost:3001/majorsroute/major/?UniversityId=${id2}`)
    ...
    useEffect(() => {
        getUserData().then((data) => {
            dispatch(actionCreator.fetchUniv()).then(() => {
                // console.log(universities, `kampus`);
                ...
                <div className="login-container">
                    ...
                    <div className="form-container">
                        <form onSubmit={(e) => submitHandler(e)} className="form-component">
                            ...
                            />
                            <select onChange={(e) => setAdminObj({ ...adminObj, univ1: e.target.value })} value={adminObj.univ1} className="form-select" aria-label="Default select example">
                                {
                                    universities.map(el => <option value={el.id}>{el.name}</option>)
                                }
                            </select>
                            <select onChange={(e) => setAdminObj({ ...adminObj, major1: e.target.value })} value={adminObj.major1} className="form-select" aria-label="Default select example">
                                {
                                    listMajor1.map(el => <option value={el.id}>{el.name}</option>)
                                }
                            </select>
                            <select onChange={(e) => setAdminObj({ ...adminObj, univ2: e.target.value })} value={adminObj.univ2} className="form-select" aria-label="Default select example">
                                {
                                    universities.map(el => <option value={el.id}>{el.name}</option>)
                                }
                            </select>
                            <select onChange={(e) => setAdminObj({ ...adminObj, major2: e.target.value })} value={adminObj.major2} className="form-select" aria-label="Default select example">
                                {
                                    listMajor2.map(el => <option value={el.id}>{el.name}</option>)
                                }
                            </select>
